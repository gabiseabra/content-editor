...
describe("useBlockNavigationPlugin", () => {
  ...
  it("moves caret to next block on ArrowDown from empty block", () => {
    cy.mount(
      <RichTextEditor id="test" autoCommit={200} value={[p(1), p(2), p(3)]} />,
    );
  ...
  it("moves caret to next block on ArrowDown from multi-line block", () => {
    cy.mount(
      <RichTextEditor
        id="test"
        autoCommit={200}
        value={[p(1, span("Multi\nLine\nString")), p(2, span("Second"))]}
      />,
    );
  ...
  it("moves caret to previous block on ArrowUp from multi-line block", () => {
    cy.mount(
      <RichTextEditor
        id="test"
        autoCommit={200}
        value={[p(1, span("First")), p(2, span("Multi\nLine\nString"))]}
      />,
    );
  ...
  it("moves through empty block when navigating down and up", () => {
    cy.mount(
      <RichTextEditor
        id="test"
        autoCommit={200}
        value={[p(1, span("A")), p(2), p(3, span("C"))]}
        onChange={() => {}}
      />,
    );
